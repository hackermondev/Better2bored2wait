...
if(process.env['LOGGER_WEBHOOK']) LOGGER_WEBHOOK = new Webhook({ url: process.env['LOGGER_WEBHOOK'] });
...
export default class TwoBTwo extends EventEmitter {
    ...
    init() {
        ...
        this._bot.bot.afk.setOptions({ minWalkingTime: (60 * 1000), maxWalkingTime: 5 * (60 * 1000), chatMessages: ['afk uwu', 'totally not afk ;-;', 'aaaaaaaaaaffffffkkkkkkkk', 'afk bruh moment']});
    ...
    async start() {
        ...
        if(this.proxyServer._server.playerCount > 0) Object.keys(this.proxyServer._server.clients).forEach((p) => this.proxyServer._server.clients[p].end('Disconnected') )
        ...
        this.proxyServer._server.on('login', (client: ServerClient) => {
            ...
            client.on('packet', (_, meta, rawData)=>{
                this._logger.info(`PROXY: Proxying (${meta.name}) packet with length ${rawData.length}`);
        ...
        this.i = setInterval(()=>{
            ...
            if(position == -1) return this._logger.error(`Unable to get queue position`);
            if(position > 2) {
